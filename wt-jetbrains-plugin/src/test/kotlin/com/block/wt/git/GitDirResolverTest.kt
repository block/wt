...
class GitDirResolverTest {
    ...
    // --- resolveMainGitDir tests ---

    @Test
    fun testResolveMainGitDirForMainRepoReturnsOwnDotGit() {
        val dir = Files.createTempDirectory("gitdir-test")
        try {
            val dotGit = dir.resolve(".git")
            Files.createDirectory(dotGit)

            val result = GitDirResolver.resolveMainGitDir(dir)
            assertNotNull(result)
            assertEquals(dotGit, result)
        } finally {
            deleteRecursive(dir)
    ...
    @Test
    fun testResolveMainGitDirForLinkedWorktreeWalksUpToDotGit() {
        val dir = Files.createTempDirectory("gitdir-test")
        try {
            val mainRepo = dir.resolve("main-repo")
            val mainGitDir = mainRepo.resolve(".git")
            ...
            val result = GitDirResolver.resolveMainGitDir(worktree)
            assertNotNull(result)
            assertEquals(mainGitDir, result)
        } finally {
            deleteRecursive(dir)
        }
    }
    ...
    @Test
    fun testResolveMainGitDirForNonGitPathReturnsNull() {
        val dir = Files.createTempDirectory("gitdir-test")
        try {
            val result = GitDirResolver.resolveMainGitDir(dir)
            assertNull(result)
