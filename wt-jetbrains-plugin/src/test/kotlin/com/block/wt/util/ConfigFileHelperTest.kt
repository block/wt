...
class ConfigFileHelperTest {
    ...
    fun testReadConfig() {
        ...
        try {
            ...
            Files.writeString(
                confFile,
                """
                WT_MAIN_REPO_ROOT="/Users/test/.wt/repos/java/base"
            ...
            assertEquals(Path.of("/Users/test/.wt/repos/java/base"), config.mainRepoRoot)
    ...
    fun testWriteAndReadRoundTrip() {
        ...
        try {
            ...
            assertNotNull(read)
            // Name is derived from .conf filename, not from ContextConfig.name
            assertEquals("roundtrip", read!!.name)
    ...
    @Test
    fun testReadConfigDefaultsBaseBranchToMaster() {
        val dir = Files.createTempDirectory("config-test")
        try {
            val confFile = dir.resolve("nobase.conf")
            Files.writeString(
                confFile,
                """
                WT_MAIN_REPO_ROOT="/Users/test/repo"
                WT_WORKTREES_BASE="/Users/test/worktrees"
                WT_ACTIVE_WORKTREE="/Users/test/active"
                WT_IDEA_FILES_BASE="/Users/test/vault"
                """.trimIndent()
            )
    ...
    @Test
    fun testReadCurrentContextReturnsNullWhenFileMissing() {
        val file = Path.of("/tmp/nonexistent-wt-test-current")
        assertNull(ConfigFileHelper.readCurrentContext(file))
    }

    @Test
    fun testReadCurrentContextReturnsNullOnEmptyFile() {
        val dir = Files.createTempDirectory("current-test")
        try {
            val file = dir.resolve("current")
    ...
    @Test
    fun testWriteAndReadCurrentContextRoundTrip() {
        val dir = Files.createTempDirectory("current-test")
        try {
            val file = dir.resolve("current")
            ConfigFileHelper.writeCurrentContext("java", file)
            assertEquals("java", ConfigFileHelper.readCurrentContext(file))
        } finally {
            deleteRecursive(dir)
        }
    }
    ...
    @Test
    fun testReadCurrentContextReadsOnlyFirstLine() {
        val dir = Files.createTempDirectory("current-test")
        try {
            val file = dir.resolve("current")
    ...
    @Test
    fun testWriteCurrentContextCreatesParentDirectory() {
        val dir = Files.createTempDirectory("current-test")
        try {
            val file = dir.resolve("subdir").resolve("current")
            ConfigFileHelper.writeCurrentContext("go", file)
            assertEquals("go", ConfigFileHelper.readCurrentContext(file))
        } finally {
            deleteRecursive(dir)
        }
    }
