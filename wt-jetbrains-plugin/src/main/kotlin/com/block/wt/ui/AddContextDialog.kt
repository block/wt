...
class AddContextDialog(private val project: Project?) : DialogWrapper(project) {
    ...
    private val mainRepoRootLabel = JBLabel()
    ...
    private var hasExistingGitConfig: Boolean = false
    ...
    private fun rederivePaths() {
        ...
        val mainRepo = PathHelper.reposDir.resolve(name).resolve("base")

        worktreesBaseLabel.text = wBase.toString()
        ideaFilesBaseLabel.text = iBase.toString()
        mainRepoRootLabel.text = mainRepo.toString()

        val repoPathStr = repoPathField.text.trim()
        if (repoPathStr.isNotBlank()) {
            migrationInfoLabel.text = "<html>Repo will be moved to <b>$mainRepo</b> and a symlink created at <b>$repoPathStr</b></html>"
        }
    }
    ...
    override fun doValidate(): ValidationInfo? {
        ...
        if (hasExistingGitConfig) {
            return ValidationInfo("This repository already has wt config in git config", repoPathField)
        }
    ...
    override fun doOKAction() {
        ...
        // activeWorktree = original repo path (the symlink location)
        activeWorktree = repoPath
        // mainRepoRoot = where repo will be moved to
        mainRepoRoot = PathHelper.reposDir.resolve(contextName).resolve("base")
